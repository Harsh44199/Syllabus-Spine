...
const LoginPage = () => {
    ...
    // If a user is already logged in and hits this page, send them to their dashboard
    useEffect(() => {
        ...
        const checkUser = async () => {
            ...
            if (session) {
                if (session.user.email === ADMIN_EMAIL) {
                    ...
                    navigate('/dashboard'); // 👈 Synchronized with App.jsx
                }
            }
        };
        checkUser();
    ...
    const handleGoogleLogin = async () => {
        ...
        const { error } = await supabase.auth.signInWithOAuth({
            provider: 'google',
            options: {
                // Send everyone to /dashboard. App.jsx will handle the Admin switch
    ...
    return (
        <PageTransition>
            <div className="min-h-[80vh] flex items-center justify-center px-4 bg-slate-50 py-12 font-medium">
                <motion.div 
                ...
                >
                    ...
                    {/* Feedback Message */}
                    {message.text && (
                        <div className={`p-4 rounded-xl mb-6 text-sm font-bold text-center ${message.type === 'error' ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-green-50 text-green-600 border border-green-100'}`}>
                            {message.text}
                        </div>
                    )}
                    ...
                    <div className="flex items-center my-6">
                        ...
                        <span className="px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Or with email</span>
                        <div className="flex-grow border-t border-slate-200"></div>
                    </div>
                    ...
                    <form onSubmit={handleAuth} className="space-y-5">
                        <div>
                            ...
                            <input
                                type="email"
                                required
                                className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-600 focus:bg-white rounded-2xl outline-none transition-all font-semibold text-slate-900"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    required
                                    className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-600 focus:bg-white rounded-2xl outline-none transition-all font-semibold pr-12 text-slate-900"
                                    placeholder="••••••••"
                                    value={password}
                    ...
                    <div className="mt-8 text-center">
                        <button
                            ...
                            className="text-sm font-bold text-slate-400 hover:text-blue-600 transition-colors uppercase tracking-tight"
                        >

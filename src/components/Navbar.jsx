...
const Navbar = ({ session }) => {
  ...
  // Smart Routing: Decide where the Dashboard button goes based on role
  const dashboardRoute = session?.user?.email === ADMIN_EMAIL ? '/admin-control' : '/dashboard';
              ...
              <div className="ml-2 pl-6 border-l border-slate-200">
                {session ? (
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    {/* 👇 SMART DASHBOARD ROUTE APPLIED HERE */}
                    <Link to={dashboardRoute} className="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-black hover:bg-blue-600 transition-colors shadow-lg shadow-slate-200">
                      DASHBOARD
                    </Link>
                  </motion.div>
                ) : (
